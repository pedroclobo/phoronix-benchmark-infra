...
class AsmSizeResultsExtractor(ResultsExtractor):
    ...
    def plot_results(self, results_file, plot_dir):
        ...
        for i, test in enumerate(tests):
            ...
            test_size_bins = np.logspace(test_log_min, test_log_max, 150)
            ...
            # Set log scale on both axes for size plot
            ax_size.set_xscale("log")
            ax_size.set_yscale(
                "symlog", linthresh=3
            )  # Linear below 3, logarithmic above

            # Set linear scale for difference plot
            ax_diff.set_yscale("symlog", linthresh=3)
